...
const Login = ({ setLoginIn }) => {
  ...
  const CreateOrganizationFun=async()=>{
    ...
    const response=await axios.post('http://ec2-13-232-103-3.ap-south-1.compute.amazonaws.com:8999/createOrganization',{
      name:name,
      location:location,
      entity:entity,
      email:email,
  ...
  const loginUser = async () => {
    ...
    try {
      const response = await axios.post('http://ec2-13-232-103-3.ap-south-1.compute.amazonaws.com:8999/login', {
        email: email,
        password: password,
