...
function CreateUser({ handleAddUser,setAllusers }) {
 ...
 useEffect(()=>{
  const getCompanies=async()=>{
    ...
    const response=await axios.post('http://ec2-13-233-247-65.ap-south-1.compute.amazonaws.com:8999/getDealpipelineCompany',{organization:organization})
 ...
 const handleSave=async()=>{
  ...
  const team=company
  let organization=localStorage.getItem('organization')
  const response=await axios.post('http://ec2-13-233-247-65.ap-south-1.compute.amazonaws.com:8999/registerUser',{
    email:email,
    password:password,
    role:role,
  ...
  if(team!='none')
  {
    await axios.post('http://ec2-13-233-247-65.ap-south-1.compute.amazonaws.com:8999/addTeam',{
      organization:team,
      member:email,
      position:role,
