...
const Dealsourcing = ({hidenavbar}) => {
  ...
  useEffect(()=>{
   const setCompany=async()=>{
    const res=await axios.post('http://ec2-13-232-103-3.ap-south-1.compute.amazonaws.com:8999/getdealsourcingFiles',{organization:localStorage.getItem('organization')})
  ...
  const handleSearch=async()=>{
    ...
    const res=await axios.post('http://ec2-13-232-103-3.ap-south-1.compute.amazonaws.com:8999/searchdealsourcingfiles',{search:search,organization:localStorage.getItem('organization')})
    const data=res.data.data
    setCompanies(data)
  }
